var editor_func = function() {
	...
	function addRow(source, subject, predicate, object, graphName, index) {
		var myTable = document.getElementById("myTable"), ontologyBody = myTable.tBodies[0], newRow = ontologyBody
				.insertRow(ontologyBody.rows.length), attribute_0 = document
				.createAttribute("class");

		attribute_0.value = "a1";
		newRow.setAttributeNode(attribute_0);

		var cell_1 = newRow.insertCell(0), cell_2 = newRow.insertCell(1), cell_3 = newRow
				.insertCell(2), cell_4 = newRow.insertCell(3), cell_5 = newRow
				.insertCell(4), cell_6 = newRow.insertCell(5), attribute_2 = document
				.createAttribute("ondblclick"), attribute_3 = document
				.createAttribute("ondblclick"), attribute_4 = document
				.createAttribute("ondblclick");
	...
	function exportFile(value, type, name) {
		...
		if (typeof window.Blob == "function")
			...
			var BlobBuilder = window.BlobBuilder || window.MozBlobBuilder
					|| window.WebKitBlobBuilder || window.MSBlobBuilder, blobBuilder = new BlobBuilder();

			blobBuilder.append(value);
			blob = blobBuilder.getBlob(type);
		}

		var url = window.URL || window.webkitURL, blobUrl = url
				.createObjectURL(blob), anchor = document.createElement("a");
	...
	function txtLoadString(s) {
		var subject = "", predicate = "", object = "", graphName = "Empty", index = 0, fileString = s, tuples = fileString
				.split('\n'), counter = 0;
	...
	/**
	 * Grab data from the URL and load it
	 */
	...
	function txtLoad(evt) {
		var subject = "", predicate = "", object = "", graphName = "Empty", index = 0, fileString = evt.target.result, tuples = fileString
				.split('\n'), counter = 0;
	...
	function handleFileSelect(evt) {
		...
		var extensibleIndex = files[0].name.lastIndexOf("."), extensibleLength = files[0].name.length, extensible = files[0].name
				.substring(extensibleIndex + 1, extensibleLength);
	...
	function loadStart() {
		var myTable = document.getElementById('myTable'), ontologyBody = myTable.tBodies[0];
	...
	function extractAttribute(string) {
		var outputTail, quote, output;

		if (string.charAt(0) === '<') {
			outputTail = string.search(" ");
			output = '&lt;' + string.substring(1, outputTail - 1) + '&gt;';
		} else if (string.charAt(0) === '"') {
			var quote = string.substr(1).search('\"'), tempt = string.substr(1)
					.substr(quote + 1), blankIndex = tempt.search(' ');
		...
		output = output.replace(/%inquo/gim, '\"').replace(/%lt/gim, '&lt;')
				.replace(/%gt/gim, '&gt;').replace(/%blank/, ' ');
		return Array(output, outputTail, quote);
	}

	function loadItem(source, string) {
		var subjectLoad = extractAttribute(string), subject = subjectLoad[0], subjectIndex = subjectLoad[1], subjectQuote = subjectLoad[2], predicateString = (subjectIndex === 0) ? string
				.substr(subjectQuote + 3)
				: string.substr(subjectIndex + 1);

		var predicateLoad = extractAttribute(predicateString), predicate = predicateLoad[0], predicateIndex = predicateLoad[1], predicateQuote = predicateLoad[2], objectString = (predicateIndex === 0) ? predicateString
				.substr(predicateQuote + 3)
				: predicateString.substr(predicateIndex + 1);

		var objectLoad = extractAttribute(objectString), object = objectLoad[0], objectIndex = objectLoad[1], objectQuote = objectLoad[2], graphString = (objectIndex === 0) ? objectString
				.substr(objectQuote + 3)
				: objectString.substr(objectIndex + 1);

		if (source === "nq") {
			var graphLoad = extractAttribute(graphString), graphName = graphLoad[0];
		} else {
			var graphName = "Empty";
		}
	...
	function nqLoad(evt) {
		var fileString = evt.target.result, tuples = fileString.split('\n');

		for (var i = 0; i < tuples.length; i++) {
			var loadString = (unescape(tuples[i].replace(/\\u/gim, "%u")
					.replace(/\\"/gim, '%inquo').replace(/\</gim, '%lt')
					.replace(/\>/gim, '%gt').replace(/%20/gim, '%blank')
					.replace(/\^\^/gim, ''))).replace(/\\n/gim, '<br>'), tuple = loadItem(
					"nq", loadString), subject = tuple[0], predicate = tuple[1], object = tuple[2], graphName = tuple[3], index = i + 1;
	...
	function ntLoad(evt) {
		...
		for (var i = 0; i < tuples.length; i++) {
			var loadString = unescape(tuples[i].replace(/\\u/gim, "%u")
					...
					.replace(/\</gim, '%lt').replace(/\>/gim, '%gt').replace(
							/%20/gim, '%blank').replace(/\^\^/gim, '')), tuple = loadItem(
					"nt", loadString), subject = tuple[0], predicate = tuple[1], object = tuple[2], graphName = tuple[3], index = i + 1;
...

	{
		...
		$.each(data, function(i, item) {
			// addRow(item.)
			addRow("jsonld", item.Subject, item.Predicate, item.Object,
	...
	/**
	 * Note if there are URL arguments when the editor is loading, then process
	 * those Let's assume that the URL args contain triples
	 */
	processURLArgs()
	return {
		...
		loadPrefix : function() {
			...
			for (var i = 0; i < prefixList.length; i++) {
				var newRow = prefixTable.insertRow(), cell1 = newRow
		...
		addPrefixRow : function() {
			var prefixTable = document.getElementById("prefixTable"), newRow = prefixTable
					.insertRow(prefixTable.rows.length), cell1 = newRow
					.insertCell(0), cell2 = newRow.insertCell(1), cell3 = newRow
					.insertCell(2), attribute_1 = document
					.createAttribute("ondblclick"), attribute_2 = document
					.createAttribute("ondblclick");
		...
		removePrefixRow : function() {
			var checkObject = document.getElementsByName("prefixCheck"), prefixTable = document
					.getElementById("prefixTable");
		...
		applyOntology : function() {
			...
			for (var j = 0; j < prefixTable.rows.length; j++)
				prefixList[j] = new Array(
						prefixTable.rows[j].cells[0].innerHTML,
						prefixTable.rows[j].cells[1].innerHTML);
			...
			for (var i = 0; i < ontologyBody.rows.length; i++) {
				for (var j = 0; j < 4; j++) {
					for (var k = 0; k < prefixList.length; k++)
						ontologyBody.rows[i].cells[j].innerHTML = ontologyBody.rows[i].cells[j].innerHTML
								.replace(prefixList[k][1], prefixList[k][0]);
				}
			}
		},
		...
		removeOntology : function() {
			var prefixTable = document.getElementById("prefixTable"), myTable = document
					.getElementById("myTable"), ontologyBody = myTable.tBodies[0];

			for (var i = 0; i < ontologyBody.rows.length; i++) {
				for (var j = 0; j < 4; j++) {
					for (var k = 0; k < prefixList.length; k++)
						ontologyBody.rows[i].cells[j].innerHTML = ontologyBody.rows[i].cells[j].innerHTML
								.replace(prefixList[k][0], prefixList[k][1]);
				}
			}
		},
		...
		tableSort : function(object) {
			// Get the table body
			var myTable = document.getElementById("myTable"), ontologyBody = myTable.tBodies[0], buffer = [], column;
			...
			if (column !== 0)
				buffer.sort(function(item_1, item_2) {
					if (isAscendant)
						return item_1.cells[column].innerHTML
								.localeCompare(item_2.cells[column].innerHTML);
					else
						return item_2.cells[column].innerHTML
								.localeCompare(item_1.cells[column].innerHTML);
				})
			else if (isAscendant)
				buffer.sort(function(item_1, item_2) {
					return item_1.cells[column].innerHTML
							- item_2.cells[column].innerHTML;
				})
			else
				buffer.sort(function(item_1, item_2) {
					return item_2.cells[column].innerHTML
							- item_1.cells[column].innerHTML;
				})
		...
		selectAll : function(target) {
			var selectState;
			if (target === "myTable") {
				var checkObject = document.getElementsByName("tableCheck"), selectState = mytableSelectState;

				mytableSelectState = !mytableSelectState;
			} else if (target === "prefixTable") {
				var checkObject = document.getElementsByName("prefixCheck"), selectState = prefixSelectState;
		...
		public_addRow : function(source, subject, predicate, object, graphName,
				index) {
		...
		removeRow : function() {
			...
			var checkObject = document.getElementsByName("tableCheck"), myTable = document
		...
		saveToSesame : function() {

			var table = $('#myTable').tableToJSON(); // Convert the table
														// into a javascript
														// object
			var data = JSON.stringify(table);
			$("#progressbar_profile").show()
			...
			$.ajax({
				...
				url : url + "SesameWriteServlet?data=" + data,
		...
		save : function() {
			var myTable = document.getElementById("myTable"), ontologyBody = myTable.tBodies[0], valueArray = new Array();
			...
			for (var i = 0; i < ontologyBody.rows.length; i++) {
				for (var j = 1; j < 5; j++) {
					...
					var content = ontologyBody.rows[i].cells[j].innerHTML
							.replace('&lt;', '<').replace(/<br>/gim, '\n')
							.replace('&gt;', '>');

					content = escape(content);
					content = content.replace(/%u/gim, '\\u').replace(
							/\"\</gim, '&lt;').replace(/%3/gim, '\\3').replace(
							/%2/gim, '\\2');
					content = content.replace(/%/gim, '\\u00').replace(
							/\\3/gim, '%3').replace(/\\2/gim, '%2');
					content = unescape(content);
					var subcontentIndex = content.lastIndexOf('"');

					if (subcontentIndex !== -1) {
						subcontent = content.substring(1, subcontentIndex)
								.replace(/\"/gim, '\\"');
						subcontent2 = content.substring(subcontentIndex + 1,
								content.length);

						if (content.charAt(subcontentIndex + 1) === '<')
							content = content.charAt(0).concat(subcontent)
									.concat('"^^').concat(subcontent2).concat(
											content.charAt(content.length + 1));
						else
							content = content.charAt(0) + subcontent + '"'
									+ subcontent2
									+ content.charAt(content.length + 1);
					}
		...
		createTextArea : function(object) {
			if (!underEdit) {
				...
				rows_num = object.offsetHeight / 12, cols_num = object.offsetWidth / 8;

				object.innerHTML = "<textarea rows=" + rows_num + " cols="
						+ cols_num
						+ " onkeydown='editor_func.showKeyCode(event,this)'>"
						+ source + "</textarea>";
				underEdit = true;
				object.focus();
			}
		...
		showKeyCode : function(evt, object) {
			if (evt.keyCode == 13) {
				...
				if (tempt.charAt(0) === '<') {
					...
					object.parentNode.innerHTML = tempt.replace("<", "&lt")
							.replace(">", "&gt");
			}
